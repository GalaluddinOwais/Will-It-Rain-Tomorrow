...
def distribution(data, continuous_features, transformed = False):
    """
    Visualization code for displaying skewed distributions of features
    """
    
    # Create figure
    fig = pl.figure(figsize = (11,20));

    # Skewed feature plotting
    
    for i, feature in enumerate(continuous_features):
        ax = fig.add_subplot(int(len(continuous_features)/2),3, i+1)
        ax.hist(data[feature], bins = 25, color = '#C83D95')
...
def feature_plot(importances, X_train, y_train):
    ...
    pl.bar(np.arange(8), values, width = 0.6, align="center", color = '#00A000', label = "Feature Weight")
    pl.bar(np.arange(8) - 0.3, np.cumsum(values), width = 0.2, align = "center", color = '#00A0A0', label = "Cumulative Feature Weight")
